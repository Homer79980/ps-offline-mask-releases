...
- `v0.3.3-preview`：Photoshop 25.0+ 的 UXP Developer Tool 预览包
- 生产 `.ccx`：待三平台原生运行时、Windows/macOS 签名、公证和真实 Photoshop 回归完成后发布

`v0.3.3-preview` 使用 29 张白底、黑底、蓝底、近纯色和复杂背景练习图完成离线回归。默认代理输出的平均半透明边缘背景色残留由 `5.37%` 降至 `0.48%`；边框不一致的图片会拒绝本地纯色一键结果，转入精修或可选 Vision API。纯色背景模式会同时生成校正后的前景 RGB 和 Alpha，避免把烘焙白底或蓝底带到新背景。Photoshop 插件菜单仍只有“离线抠图”一个入口；主面板保留缩略图并通过“打开抠图精修”打开模态工作区。

精修窗口的画笔、滚轮缩放、空格平移和 `[`/`]` 会直接响应，不再等待一组输入事件全部通过。底部输入诊断只记录 Photoshop 实际转发了哪些事件，不会吞掉首次绘制；输入模块的自动化测试已通过，但真实 Photoshop 25+ 仍需用户按清单验收。

纯色/近纯色本地算法会处理与外边缘不连通的内部背景孔洞，并保留软 Alpha。`v0.3.3-preview` 新增“边缘收缩/扩张”和“抗锯齿柔化”：柔化提供关闭、1–4 px 五档，只向主体内部产生过渡，不会复活完全透明背景；仍有细边时可试“收缩 1 px + 柔化 1 px”。小面积背景同色区域仍优先保护为高光或内部细节，若它实际是小孔洞，需要用精修“删除”画笔修正。相近前景和背景无法可靠区分时会失败关闭，不承诺“任何图片都能抠净”。
