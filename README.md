...
- `v0.3.1-preview`：Photoshop 25.0+ 的 UXP Developer Tool 预览包
- 生产 `.ccx`：待三平台原生运行时、Windows/macOS 签名、公证和真实 Photoshop 回归完成后发布

`v0.3.1-preview` 修复彩色插画主体内部、高光和浅色纹理被错误变成半透明的问题，同时保持 Photoshop 插件菜单只有“离线抠图”一个入口。主面板保留当前图层/蒙版结果缩略图，并通过“打开抠图精修”打开大尺寸模态工作区；默认设计尺寸 `1100×820`，最小设计尺寸 `640×480`，窗口边缘调整尺寸仍需真实 Photoshop 25+ 验证。

精修窗口的画笔、滚轮缩放、空格平移和 `[`/`]` 会直接响应，不再等待一组输入事件全部通过。底部输入诊断只记录 Photoshop 实际转发了哪些事件，不会吞掉首次绘制；输入模块的自动化测试已通过，但真实 Photoshop 25+ 仍需用户按清单验收。

纯色/近纯色本地算法会处理与外边缘不连通的内部背景孔洞，并保留抗锯齿软 Alpha。`v0.3.1-preview` 对小面积背景同色区域优先保护为高光或内部细节，避免水果等彩色主体被掏空；若它实际是小孔洞，需要用精修“删除”画笔修正。相近前景和背景无法可靠区分时会失败关闭，不承诺“任何图片都能抠净”。单图和批量共用同一算法入口；默认只写 Alpha 蒙版。
