...
- `v0.3.2-preview`：Photoshop 25.0+ 的 UXP Developer Tool 预览包
- 生产 `.ccx`：待三平台原生运行时、Windows/macOS 签名、公证和真实 Photoshop 回归完成后发布

`v0.3.2-preview` 使用用户提供的 `840×840` 水果总览原图完成离线回归：保留九个彩色主体，清除樱桃梗之间的浅灰/纯白混色孔洞，并释放与边界连通的浅色残片。纯色背景模式会同时生成校正后的前景 RGB 和 Alpha，避免把烘焙白底带到深色背景。Photoshop 插件菜单仍只有“离线抠图”一个入口；主面板保留缩略图并通过“打开抠图精修”打开模态工作区。

精修窗口的画笔、滚轮缩放、空格平移和 `[`/`]` 会直接响应，不再等待一组输入事件全部通过。底部输入诊断只记录 Photoshop 实际转发了哪些事件，不会吞掉首次绘制；输入模块的自动化测试已通过，但真实 Photoshop 25+ 仍需用户按清单验收。

纯色/近纯色本地算法会处理与外边缘不连通的内部背景孔洞，并保留抗锯齿软 Alpha。`v0.3.2-preview` 对小面积背景同色区域优先保护为高光或内部细节，避免水果等彩色主体被掏空；若它实际是小孔洞，需要用精修“删除”画笔修正。相近前景和背景无法可靠区分时会失败关闭，不承诺“任何图片都能抠净”。单图和批量共用同一算法入口；可靠纯色背景会输出校正前景层与 Alpha，无可靠背景的经典精修保持仅蒙版。
