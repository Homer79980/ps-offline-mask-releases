...
`v0.2.1-preview` 修复并补全实际编辑流程：预览图可在 5%–800% 间自由缩放；未知、保留和删除工具会显示圆形画笔并从第一笔开始生效；“应用蒙版到图层”固定放在主操作区；在 Photoshop 图层面板多选像素图层后可直接批量生成并应用蒙版。
...
从旧版本升级时，请在 UXP Developer Tool 中 Stop 旧实例，重新解压当前 Release 包，然后 Add/Load 新目录中的 `manifest.json`。
