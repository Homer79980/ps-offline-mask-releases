...
`v0.2.5-preview` 修复了 `v0.2.4` 中会让所有按钮失效的 UXP 生命周期初始化错误，并在“离线抠图”主面板恢复只读缩略图：读取后显示当前图层，计算后显示蒙版结果。点击主面板的“打开抠图精修工作区”即可进入可停靠、浮动和自由拉伸的完整画布。

Photoshop 会自动把每个原生 UXP 面板列入插件菜单，因此菜单中仍会同时出现“离线抠图”和“抠图精修”。后者是主面板自动打开失败时的备用入口；UXP 没有在保留独立可拉伸面板时隐藏它的官方配置。
...
从旧版本升级时，请在 UXP Developer Tool 中 Stop 并移除旧实例，重新解压当前 Release 包，然后 Add/Load 新目录中的 `manifest.json`。不要覆盖正在加载的旧目录。本版 108 项自动化测试与发布审计已通过；按用户要求，Photoshop 24.4 中的主面板唤起、缩略图、画笔、滚轮和平移由用户安装后真机复测。
