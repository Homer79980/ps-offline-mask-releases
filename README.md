...
`v0.2.6-preview` 把 Photoshop 插件菜单收敛为唯一入口“离线抠图”。主面板保留当前图层/蒙版结果缩略图，并通过“打开抠图精修窗口”按钮打开 `1100×820` 的大尺寸模态工作区，不再注册第二个“抠图精修”菜单项。

精修画布支持围绕鼠标位置滚轮缩放、空格加左键平移，以及未知/保留/删除三种半透明双圈画笔。`[` / `]` 调整画笔直径，工具栏和光标旁的 `px` 标签实时显示大小；柔软度会真实写入 Trimap 的 UNKNOWN 过渡区域。重新计算会更新插件预览，“应用蒙版”是单图层流程唯一写入 Photoshop 的动作，取消或关闭精修窗口会恢复进入精修时的内存状态。
...
从旧版本升级时，请在 UXP Developer Tool 中 Stop 并移除旧实例，重新解压当前 Release 包，然后 Add/Load 新目录中的 `manifest.json`。不要覆盖正在加载的旧目录；否则旧 manifest 仍会显示两个菜单入口。本版 110 项自动化测试、语法检查、发布审计和两路独立代码复审已通过。按用户要求，本轮没有自动操作 Photoshop；Photoshop 24.4 中的菜单、画笔和应用流程由用户安装后真机复测。
