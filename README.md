...
- `v0.4.0-preview`：Photoshop 25.0+ 的 UXP Developer Tool 预览包
- 生产 `.ccx`：待三平台原生运行时、Windows/macOS 签名、公证和真实 Photoshop 回归完成后发布

`v0.4.0-preview` 把缩略图放大和精修合并为一个 Photoshop 风格工作区。未知、保留、删除画笔直接在原图上绘制，支持滚轮缩放、空格平移、`[`/`]` 调整直径、柔软度、Esc 取消当前笔迹，以及按操作顺序撤销/重做整笔和边缘参数。

主面板与工作区共享同一份基础 Alpha 和实时边缘参数。透明、白、黑、灰和自定义背景都显示真实彩色抠图效果；质量检查会显示引擎、策略、置信度和边缘风险。连续拖动会中止旧计算，显示栅格最长边限制为 2048 像素，原始分辨率仍用于计算和写回。317 项自动化测试已通过，但真实 Photoshop 25+ 仍需用户按清单验收。
...
图层批量固定使用本地算法并严格串行。文件夹批量可选择源目录和输出目录，支持 PNG/JPG/JPEG/WebP、递归扫描、相对目录保留、透明 PNG 自动编号、安全停止和 JSON 报告。非像素、背景和完全锁定图层会跳过；低置信度结果失败关闭，单项失败不会覆盖已有输出。
