...
- `v0.3.4-preview`：Photoshop 25.0+ 的 UXP Developer Tool 预览包
- 生产 `.ccx`：待三平台原生运行时、Windows/macOS 签名、公证和真实 Photoshop 回归完成后发布

`v0.3.4-preview` 取代 `v0.3.3` 的平均残留指标：新版按单图扫描透明外部可达色边、同色投影、最大残留连通块、近不透明色块和封闭近背景色块，任何算法适用图片失败都会阻止整批通过。29 张本地练习图中，23 张算法适用图逐张通过；3 张边框不一致图片明确不适用；3 张存在纯 RGB 固有歧义（包括人物蓝底图），一键抠图会停止并要求精修或 Vision API，不再输出带蓝块的假成功蒙版。纯色背景模式仍会同时生成校正后的前景 RGB 和 Alpha。Photoshop 插件菜单只有“离线抠图”一个入口；主面板保留缩略图并通过“打开抠图精修”打开模态工作区。

精修窗口的画笔、滚轮缩放、空格平移和 `[`/`]` 会直接响应，不再等待一组输入事件全部通过。底部输入诊断只记录 Photoshop 实际转发了哪些事件，不会吞掉首次绘制；输入模块的自动化测试已通过，但真实 Photoshop 25+ 仍需用户按清单验收。

纯色/近纯色本地算法会处理与外边缘不连通的内部背景孔洞，并保留软 Alpha。`v0.3.4-preview` 的本地一键默认收缩 `1 px`，抗锯齿柔化仍提供关闭、1–4 px 五档且默认关闭。处于测得背景色容差带内、且达到面积门槛的封闭区按孔洞删除；若主体确有同容差色装饰，需在精修窗口用“保留”恢复。高饱和背景上的深色同色投影和 JPEG 偏色边缘不再直接升级为不透明前景；超出背景容差的显著近色歧义会失败关闭并要求精修或可选 Vision API，不承诺“任何图片都能抠净”。
