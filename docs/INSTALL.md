...
当前公开下载是 `v0.3.0-preview` UXP Developer Tool 开发预览包，不是签名后的生产 `.ccx`。最低宿主为 Photoshop 25.0。纯色背景算法已通过无宿主自动化质量门；精修输入、前景图层写回、批量宿主写回和 Windows/macOS 安装仍需真实宿主验收。
...
本预览版只以 Photoshop 25+ 的直接 Imaging 写入为发布目标。仓库中的旧兼容代码不属于 v0.3.0-preview 支持声明。
