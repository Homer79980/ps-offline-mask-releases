...
如果面板能显示但按钮、滑杆和预览都没有反应，通常是旧包仍驻留在 UXP 中：先点击 **Stop**，再对当前解压目录中的 `manifest.json` 执行 **Load**。不要只重新打开旧面板。

面板支持在 Photoshop 中拖拽浮动窗口边缘调整大小，宿主允许的范围为约 `320×420` 到 `1600×2400`。如果旧实例仍保持旧尺寸，请在 UXP Developer Tool 中 Stop 后重新 Load 一次。

开发预览包只处理当前文档中选中的像素图层；单图模式读取首个选中像素图层，批量模式处理全部选中像素图层。文字、智能对象、组和矢量图层请先栅格化。
